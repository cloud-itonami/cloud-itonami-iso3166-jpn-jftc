...
2. Register the client's intake: business type, the specific
   JFTC-regulated activity involved, prior filing/compliance
   history in Japan if any.
3. Run the advisor in read-only mode against Fair Trade Commission's
   (公正取引委員会) published guidance.
...
5. Enable gated filing/compliance-draft assistance once the
   Antitrust Compliance Governor contract is trusted; actual submission always
   requires human sign-off.

## Minimum Production Controls

- client-owned data store for compliance documents
- clear provenance (official JFTC source citation) for every
...
automated actions cannot bypass the Antitrust Compliance Governor, and a working
